...
int main()
{
    ...
    while(m--)
    {
        ...
        {
            ll t=vec[i];
            //cout<< "vec[i] " << vec[i] << " dp[vec[i]] " << dp[vec[i]] << "\n";
            if(dp[t]>=8){ok1=1;ok2=1;break;}
            if(dp[t]>=4 && !ok1){x=dp[t]-4,ok1=1;/*cout<<"x "<<x << "ok1 "<< ok1 << " ";*/if(x>=2){cnt++;}}
            else if(dp[t]>=4 && ok1){ok2=1;break;}
            else if( !ok2 && dp[vec[i]]>=2){
                cnt++;//cout<<"cnt " << cnt << " ";
                if(cnt>=2 ){
                    //cout << "bal " ;
                    ok2=1;
