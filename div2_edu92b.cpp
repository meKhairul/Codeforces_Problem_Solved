...
//-----------------------------
ll n,k,z,a[100005],ans,pos,mx,t;
map<ll,ll>sum;
...
int main()
{
    Test{
        ...
        cin3(n,k,z);ans=0;sum.clear();mx=0;
        cin>>a[0];sum[0]=a[0];
        for(int i=1;i<n;i++)
        {
            cin>>a[i];
        ...
        //cout<< "pos : " << pos << "mx : " << mx  << "sum : " << sum[pos]<< "\n";
        if(z==0 || pos==k)ans=sum[k];
        else
        {
            k-=pos;//cout<< "k : " << k << "\n";
                t=(k+1)/2;//cout << " t : " << t << "\n";
                ...
                {
                    if(k==1)ans=sum[pos]+a[pos-1];
                    else{
                            //cout<< "k : " << k << "\n";
                        ans=sum[pos]+mx*(k/2)+((k%2)*a[pos-1]);
