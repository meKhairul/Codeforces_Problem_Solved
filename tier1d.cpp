...
void bal()
{
    ...
    auto high = lower_bound(vec.begin(),vec.end(),y)-vec.begin();
    //cout<<"high :"<<high<< " vec[high] "<< vec[high] <<"\n";
    if(vec[high]>y || vec[high]==0 || y>g)high--;
    //cout<<"high :"<<high<<"\n";
    if(vec[high]<x)coul("-1");
    else coul(vec[high]);
}
