...
//-----------------------------
ll n,m,u,v,p[100005],h[100005],peo[100005],ok,sum;
...
void solve()
{
    for(int i=2;i<=n;i++)
    {
        for(vector<ll>::iterator it = adj[i].begin();it!=adj[i].end();it++)
        {
            //cout << "*it : " << *it << " p[*it] : " << p[*it] << "\n";
            peo[i]+=p[*it];
        }
    }
    ...
    {
        cout << peo[i] << " " << h[i] << "\n";
        if(peo[i]==h[i] && h[i]==0)continue;
        else if(peo[i]%2)
        {
            if(abs(h[i])%2==0 || peo[i]<abs(h[i])){/*cout<<"i1 : " << i <<" "; */ok = 1;break;}
        }
        else
        {
            if(abs(h[i])%2==1 || peo[i]<abs(h[i])){/*cout<<"i2 : " << i <<" "; */ok = 1;break;}
        }
    }
    if(!ok)coul("YES");
    else coul("NO");
...
int main()
{
    Test{
        ...
        cin>>n>>m;ok=0;
        for(int i=1;i<=n;i++){cin>>p[i];peo[i]=p[i];adj[i].clear();}
