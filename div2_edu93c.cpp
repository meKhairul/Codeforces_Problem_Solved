...
ll n,m,ans,sum[100005],dif[100005],dif2[100005],cnt,one,zero;
string s;
...
int main()
{
    Test{
        ...
        for(int i=1;i<n;i++)
        {
            ...
            sum[i] = sum[i-1]+(s[i]-'0');
            dif[i]=(sum[i]-sum[i-1]);
            dif2[i]=(s[i]-'0')+(s[i-1]-'0');
            //cout << " sum[i] " << sum[i] << "\n";
            //cout << " dif[i] " << dif[i] << "\n";
            //cout << " dif2[i] " << dif2[i] << "\n";
            if(dif[i]==1)cnt++;
            if(dif2[i]==2)cnt++;
            if( i!=1 && sum[i]==i+1)cnt++;
        }
        if(one==n)
        {
            cnt=(n*(n+1))/2;
        }
        else if(zero==n)cnt=0;
        else{
        for(int i=3;i<=n-1;i++)
        {
            for(int j=0;j+i<n;j++)
            {
                //cout << " i: "<< i << " j: " << j << " abs : " << sum[j+i] - sum[j] << " \n";
                if((j+i)<n && sum[j+i]-sum[j]==i)cnt++;
            }
        }
        }
        cout << cnt << "\n";
